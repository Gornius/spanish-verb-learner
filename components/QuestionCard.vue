...
const previousAnswer = computed(
  () => storeQuiz.previousAnswers.toReversed()[0],
);
const previousAnswerCorrect = computed(() =>
  wordComparator.compareWords(
    previousAnswer.value.answer,
    previousAnswer.value.verb.word,
  ),
);
...
<template>
  <form @submit.prevent="giveAnswer">
    ...
    >
      <CardContent
        class="grow pt-6 flex flex-col md:max-w-96 gap-2 content-center justify-center"
      >
        ...
        <div class="text-lg text-center">
          ...
          {{ wordsDictionary.getPerson(props.verb) }}
        </div>
      </CardContent>
