...
export const useStoreSettings = defineStore("settings", () => {
  ...
  const pickedVerbsTypes = ref<CheckedVerbsList[]>([
    {
      verbs: wordsDictionary.irregularVerbsList,
      checked: true,
      name: "Irregular",
    },
    { verbs: wordsDictionary.regularVerbsList, checked: true, name: "Regular" },
  ]);
  const verbsPool = computed(() =>
    pickedVerbsTypes.value.reduce((pool, current) => {
      return current.checked ? [...pool, ...current.verbs] : pool;
    }, [] as Verb[]),
  );
  const pickedTimes = ref(
    (() =>
      wordsDictionary
        .getAvailableTimes(verbsPool.value)
        .map((time) => ({ checked: true, name: time }) as CheckedTime))(),
  );
  const pickedPersons = ref(
    (() =>
      wordsDictionary
        .getAvailablePersons(verbsPool.value)
        .map((time) => ({ checked: true, name: time }) as CheckedPerson))(),
  );
